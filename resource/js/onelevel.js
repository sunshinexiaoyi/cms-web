/**
 * Name:onelevel.js
 * Author:Van
 * E-mail:zheng_jinfan@126.com
 * Website:http://kit.zhengjinfan.cn/
 * LICENSE:MIT
 * 顶部 一级菜单
 */
layui.define(['jquery', 'laytpl', 'element'], function(exports) {
    var $ = layui.jquery,
        _modName = 'oneLevel',
        _win = $(window),
        _doc = $(document),
        laytpl = layui.laytpl;

    var oneLevel = {
        v: '1.0.1',
        config: {
            elem: undefined,
            data: undefined,
            remote: {
                url: undefined,
                type: 'POST'
            },
            onClicked: undefined
        },
        set: function(options) {
            var that = this;
            $.extend(true, that.config, options);
            return that;
        },
        /**
         * 是否已设置了elem
         */
        hasElem: function() {
            var that = this,
                _config = that.config;
            if (_config.elem === undefined && _doc.find('ul[kit-one-level]').length === 0 && $(_config.elem)) {
                //console.log('One-Level error:请配置One-Level容器.');
                //do something..
                return false;
            }
            return true;
        },
        /**
         * 获取容器的jq对象
         */
        getElem: function() {
            var _config = this.config;
            return (_config.elem !== undefined && $(_config.elem).length > 0) ? $(_config.elem) : _doc.find('ul[kit-one-level]');
        },
        render: function() {
            var that = this,
                _config = that.config, //配置
                _remote = _config.remote, //远程参数配置
                _data = [];

            if (!that.hasElem())
                return that;

            //本地数据优先
            if (_config.data !== undefined && _config.data.length > 0) {
                _data = _config.data;
            } else {
                var dataType = _remote.jsonp ? 'jsonp' : 'json';
                var options = {
                    url: _remote.url,
                    type: _remote.type,
                    error: function(xhr, status, thrown) {
                        layui.hint().error('One-Level error:AJAX请求出错.' + thrown);
                    },
                    success: function(res) {
                        _data = res;
                        oneLevel.loadSuccess(res);
                    }
                };
                $.extend(true, options, _remote.jsonp ? {
                    dataType: 'jsonp',
                    jsonp: 'callback',
                    jsonpCallback: 'jsonpCallback'
                } : {
                    dataType: 'json'
                });
                $.support.cors = true;
                $.ajax(options);
            }
            return that;
        },
        loadSuccess:function(data){
            console.info("服务器返回菜单数据");
            console.info(data);

            var menuData =  oneLevel.format(data);
            if(null != menuData){
                oneLevel.updateView(menuData);
            }
        },
        updateView:function(data){
            var  _tpl = [
                '{{# layui.each(d,function(index, item){ }}',
                '<li class="layui-nav-item">',
                '<a href="javascript:;" data-title="{{item.menuname}}" data-icon="{{item.icon}}" data-id="{{item.menuid}}" >',
                '{{# if (item.icon.indexOf("fa-") !== -1) { }}',
                '<i class="fa {{item.icon}}" aria-hidden="true"></i>',
                '{{# } else { }}',
                '<i class="layui-icon">{{item.icon}}</i>',
                '{{# } }}',
                '<span> {{item.menuname}}</span>',
                '</a>',
                '</li>',
                '{{# }); }}',
            ]; //模板
            var _elem = this.getElem();
            var navbarLoadIndex = layer.load(2);
            var _config = this.config;

            //渲染模板
            laytpl(_tpl.join('')).render(data.oneLevel, function(html) {
                _elem.html(html);
                layui.element.init();
                typeof _config.onClicked === 'function' && _elem.children('li.layui-nav-item').off('click').on('click', function() {
                    var _a = $(this).children('a'),
                        id = _a.data('id');
                    //console.info("菜单数据");
                    //console.info(id);
                    _config.onClicked(data.towLevel[""+id]);
                });
                //关闭等待层
                navbarLoadIndex && layer.close(navbarLoadIndex);
                typeof _config.renderAfter === 'function' && _config.renderAfter(_elem);
            });
        },
        /*对接收的数据格式化处理*/
        format:function(data){
            if(data.code == "200"){
                var menuData = {
                    oneLevel:[],
                    towLevel:{}
                };
                var list = data.listData;
                for(var i=0;i<list.length;i++){
                    list[i].id = list[i].menuid;        //必须添加id，用于tab切换
                    var parentid = list[i].parentid ;
                    if(null == parentid){       //第一级菜单
                        menuData.oneLevel.push(list[i]);
                    }else{
                        parentid +='';  //变成字符串,关联数组索引
                        if("undefined" == typeof menuData.towLevel[parentid]){//不存在则新建数组
                            menuData.towLevel[parentid] = [];
                        }
                        menuData.towLevel[parentid].push(list[i]);//添加
                    }
                }

               // console.info("格式化的菜单数据");
                //console.info(menuData);
                return menuData;

            }else {
                //console.error('data.code != "200"');
                layui.hint().error('data.code != "200"' );
                return null;
            }
        }
    };

    exports('onelevel', oneLevel);
});