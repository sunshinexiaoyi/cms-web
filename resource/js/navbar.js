...
layui.define(['layer', 'laytpl', 'element'], function(exports) {
        ...
        render: function(callback) {
            var that = this,
                ...
                _tpl = [
                    ...
                    '<dd>',
                    '<a href="javascript:;" kit-target data-options="{url:\'{{child.menuurl}}\',icon:\'{{child.icon}}\',title:\'{{child.menuname}}\',id:\'{{child.id}}\'}">',
                    ...
                    '</dl>',
                    '{{# }else{ }}',
                    '<a href="javascript:;" kit-target data-options="{url:\'{{item.menuurl}}\',icon:\'{{item.icon}}\',title:\'{{item.menuname}}\',id:\'{{item.id}}\'}">',
                    '{{# if (item.icon.indexOf("fa-") !== -1) { }}',
                    '<i class="fa {{item.icon}}" aria-hidden="true"></i>',
                    '{{# } else { }}',
                    '<i class="layui-icon">{{item.icon}}</i>',
    ...
    function slideListener(){
        var _side = _doc.find('div.layui-layout-admin');
        $(".site-tree-mobile").off("click").on("click",function(){
            slideToggle(_side);

        });

        $(".site-mobile-shade").off("click").on("click",function(){
            _side.removeClass('site-mobile');
        });
    }

    function slideToggle($ele){
        $ele.toggleClass('site-mobile');
    }
