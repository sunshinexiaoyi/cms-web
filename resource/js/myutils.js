...
layui.define(['layer'], function(exports) {
    ...
    var utils = {
        ...
        //语言设置
        setLang:function (dict){
            for (var item in dict) {
                var field = dict[item].field;
                var $filedEle = $("[name='" + field + "']");
                if (0 != $filedEle.length) {
                    var $langEle = $filedEle.parent().prev().filter('label');
                    if (0 != $langEle.length) {
                        $langEle.html(dict[item].title + ":");
                    }
                }
            }
        },
                ...
                if($ele.length)//元素存在
                {
                    ...
                    switch($ele.prop("tagName"))
                    {
                        ...
                        case "H3":
                            ...
                            break;
                        case  "SELECT":
                            $ele.next().find("[lay-value='"+value+"']").click();
                            console.info($ele.next().find("[lay-value='"+value+"']"));//模拟点击下拉框
                            break;
                        case "INPUT":
                        case "TEXTAREA":
                            ...
                            if("checkbox" == type){
                                ...
                                var $img = $ele.next().find("img"); //图片
                                //console.info($ele.next());
                                //console.info($img);
                                if($img.length){
                                    $img.attr("src",value);
                                }
